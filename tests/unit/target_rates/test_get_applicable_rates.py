...
from custom_components.carbon_intensity.target_rates import get_applicable_rates
...
async def test_when_continuous_times_present_then_next_continuous_times_returned(current_date, target_start_time, target_end_time, expected_first_valid_from, is_rolling_target, expected_number_of_rates):
  ...
  for item in result:
    assert item["from"] == expected_first_valid_from
    assert item["to"] == expected_first_valid_from + timedelta(minutes=30)
    expected_first_valid_from = item["to"]
