...
from custom_components.carbon_intensity.target_sensor_utils import calculate_intermittent_times
...
async def test_when_offset_set_then_next_continuous_times_returned_have_offset_applied():
  ...
  assert result != None
  assert len(result) == 2
  assert result[0]["from"] == datetime.strptime("2022-02-09T12:00:00Z", "%Y-%m-%dT%H:%M:%S%z")
  assert result[0]["to"] == datetime.strptime("2022-02-09T12:00:00Z", "%Y-%m-%dT%H:%M:%S%z") + timedelta(minutes=30)
  assert result[0]["intensity_forecast"] == 0.1

  assert result[1]["from"] == datetime.strptime("2022-02-09T13:30:00Z", "%Y-%m-%dT%H:%M:%S%z")
  assert result[1]["to"] == datetime.strptime("2022-02-09T13:30:00Z", "%Y-%m-%dT%H:%M:%S%z") + timedelta(minutes=30)
  assert result[1]["intensity_forecast"] == 0.1